...
| Background jobs | DB-trigger outbox (`notification_queue`) drained by cron-hit endpoints (`/api/cron/*`) + Next.js `after()`. No external job runner — chosen for VPS portability. |
| Logs | Axiom (or Better Stack) |
| Hosting | Hostinger VPS (`187.77.9.149`) via **Dokku** — app `dastify-connect`, Docker/Dockerfile build, Next.js standalone. Coexists with other Dokku apps on the box. Was Vercel in early phases; see `docs/SETUP.md` §8. |
| Pkg manager | pnpm |
...
19. **State transitions are validated server-side, but admins may move to ANY status directly.** The linear path in rule 17 is the *happy path*, not a constraint — `validateTransition` rejects only a no-op (status unchanged); every distinct from→to is allowed (stages can be skipped, terminal states re-opened). Each transition still writes one row to `status_history` (append-only, enforced by `trg_status_history_no_update`).
...
/lib/notifications → Outbox render + drain worker + digest (background email)
/app/api/cron      → Cron-hit endpoints (notifications drain, digests)
/tests
  /unit            → Vitest
  /e2e             → Playwright
```
...
- ❌ Sending an email from anywhere other than the Graph wrapper in `/lib/email`. All transactional mail goes through `sendEmail(...)` in `lib/email/client.ts`, which uses Microsoft Graph `sendMail` with app-only OAuth — no nodemailer, no Resend, no direct `fetch` to Graph from anywhere else. This includes Supabase Auth email, which is routed through the same wrapper via the Send Email Hook (`app/api/auth/send-email`), not Supabase's built-in SMTP. If Graph breaks and the emergency SMTP fallback is activated, the wrapper swap also lives in `lib/email/client.ts` (nodemailer version is in git history); do not split the surface.
- ❌ Adding a third-party email-templating dependency without §2 approval. HTML email bodies are built by a hand-rolled, **dependency-free** layout module in `lib/email/` (TypeScript string templates, table-based markup, fully inline styles, audience-aware deep links) — no template engine, no component library. If cross-client rendering robustness ever justifies more, the **documented future upgrade paths** (both free/MIT but third-party deps, so each needs explicit §2 sign-off before being added) are: (a) `@react-email/components` + `@react-email/render` — React components rendered to static HTML, strong Outlook handling, nicer composition; (b) `mjml` — responsive email compiler/toolchain. Neither changes egress; both still send through the Graph wrapper above.
...
- [ ] **Enable Supabase Auth Send Email Hook** — routes auth emails (invite / magic link / recovery / signup / email change) through our Graph wrapper instead of SMTP, so *all* email is on Graph/OAuth (no SMTP password anywhere). Code is built: `app/api/auth/send-email/route.ts` + `lib/email/auth-templates.ts`, route made public in `middleware.ts`. To turn on: Supabase dashboard → Auth → Hooks → Send Email Hook → HTTPS type → URL `https://<prod-host>/api/auth/send-email` → copy the generated `v1,whsec_…` secret into `SEND_EMAIL_HOOK_SECRET` (env + redeploy) → enable the hook → **disable custom SMTP** so Supabase doesn't double-send. Hook URL must be publicly reachable (no localhost — use a deployed URL or tunnel). Caveat: if the app deployment is down, auth emails fail. Full procedure in `docs/SETUP.md` §4.
...
- [ ] **Set `CRON_SECRET` in the host env** — required by `/api/cron/notifications` and `/api/cron/digest` (bearer auth). On Vercel it's already added; Vercel Cron injects `Authorization: Bearer <CRON_SECRET>` automatically. On the VPS, set it in the process env and have the crontab send the same header.
- [ ] **VPS crontab (post-migration)** — when off Vercel, drive the schedules with system cron, e.g. `* * * * * curl -fsS -H "Authorization: Bearer $CRON_SECRET" https://<host>/api/cron/notifications` (every minute) and `0 14 * * * curl -fsS -H "Authorization: Bearer $CRON_SECRET" https://<host>/api/cron/digest` (daily; the endpoint runs the weekly digest itself on Mondays). On Vercel the equivalent lives in `vercel.json` (limited to daily on Hobby — the `after()` immediate drain covers real-time delivery there).
