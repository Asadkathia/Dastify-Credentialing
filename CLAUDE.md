...
| Email | Office 365 SMTP via nodemailer (`digital@dastifysolutions.com`) |
...
- ❌ Sending an email from anywhere other than the SMTP wrapper in `/lib/email`. Office 365 SMTP via nodemailer — no Resend, no direct `nodemailer.createTransport(...)` calls outside `lib/email/client.ts`.
...
- [ ] **Email sender (Supabase Auth)** — Supabase's default sender is rate-limited (~3/hr) and brands as "supabase.io". Before client-facing emails, configure custom SMTP under Auth → SMTP Settings using the **same Office 365 mailbox** the app uses (`digital@dastifysolutions.com`, `smtp.office365.com:587`, STARTTLS, app password). This keeps every email — invites, magic links, status notifications, digests — coming from one sender so SPF/DKIM/DMARC and reply-handling stay consistent.
- [ ] **Office 365 mailbox setup for SMTP** — for `digital@dastifysolutions.com`: (a) Exchange admin center → mailboxes → select the mailbox → Manage email apps → enable **Authenticated SMTP**; (b) if MFA is on (it should be), generate an **app password** from the user's Microsoft account → Security → Advanced security options, and use that as `SMTP_PASS` (never the user's login password). Sending caps to plan around: ~30 messages/min, ~10,000 recipients/day per mailbox. No bounce/complaint webhooks — monitor the mailbox's inbox for delivery failures.
