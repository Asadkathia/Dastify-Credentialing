...
8. **An enrollment is keyed by `(client_id, [provider_id|group_entity_id], payer_id, state, cycle_number)`.** Don't deduplicate by `(provider, payer)` alone — state and cycle are first-class.
9. **Recredentialing creates a NEW enrollment row** linked via `parent_enrollment_id`. Never reset status on the existing row.
10. **An enrollment has exactly one of `provider_id` or `group_entity_id`** — enforced by a CHECK constraint.

### Status lifecycle

11. **Status is a closed enum**: `intake`, `prep`, `submitted`, `in_review`, `info_requested`, `approved`, `denied`, `effective`, `closed`, `withdrawn`. Free-form nuance lives in `sub_status` (text), not in invented enum values.
12. **State transitions are validated server-side.** A transition that violates the documented machine returns a 400 with an explanation; it does not silently succeed.
13. **Setting status to `effective` sets `effective_date` and computes `next_recred_due_date`** (default 24 months unless payer overrides). This is automatic, not manual.

### UX bars

14. **Every list view is paginated.** Never render an unbounded query result.
15. **Every destructive action requires confirmation** — soft delete by default; hard delete is admin-only and audit-logged.
16. **The .xlsx export must reproduce the existing template format exactly**: banner row (configurable), `States | Payers | Participation Request Status | Comments` header, one row per enrollment. The user has clients trained on this layout — do not "improve" it without permission.
...
- Auth + admin-provisioned client invites (Supabase Auth, magic-link).
- Multi-tenant data model per `docs/DESIGN.md` §3.
- Admin portal: full CRUD on clients, providers, group entities, payers, enrollments, comments, internal notes, documents.
- Client portal: read access scoped via RLS to their own client's data; comment posting; .xlsx export.
- Status pipeline (7 stages + sub-status + closed/withdrawn) with server-side transition validation.
- Recred auto-creation Inngest job (90 days before due).
- Documents with category, expiration tracking, internal/public flag, virus scanning hook.
- Audit log: `status_history` + `activity_events`, append-only, visible per-enrollment.
- .xlsx export matching the existing Excel template.
- Email notifications (Resend): status change, client-comment-to-admin, daily/weekly digest, expiration alerts.
- Configurable per-client disclaimer banner.
- Login + audit-logged sessions.

### Out of scope for v1 — push back if asked

AI/agent features, real-time collaboration, PDF export, in-app notification bell, SMS, SAML/OIDC SSO, CAQH/NPPES integrations, public API/webhooks, mobile apps, multi-tier admin roles, white-label branding, bulk import.
